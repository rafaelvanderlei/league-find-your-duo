...
export const getSummonerById = summonerId => {
  return getWithThrottling(`/summoner/v3/summoners/${summonerId}`)
    .catch(stdCatch(`error trying to get summoner ${summonerId}.`))
}

export const getSummonerMatches = accountId => {
  console.log(`requesting matches for ${accountId}`)
  return getWithThrottling(`/match/v3/matchlists/by-account/${accountId}`)
    .catch(stdCatch(`error trying to get matches for summoner ${accountId}.`))
}
